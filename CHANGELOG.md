...
- 🧩 **`MqttClient` structural protocol** (HIGH H5). New `mqtt_client.py` declares the minimum paho surface — `on_connect`/`on_disconnect` properties, `will_set`, `connect_async`, `loop_start`/`stop`, `disconnect`, `publish`, `enable_logger`, `username_pw_set`, `reconnect_delay_set`, `max_queued_messages_set`. `_run` is typed against the Protocol, `main()` casts the real paho client at the boundary, and `FakeMqttClient` structurally satisfies it without importing production code. Delivers type coverage across the previously-untyped paho seam
- 🧾 **`monitor.py` audit records typed via `AuditRecord` TypedDict.** Flat schema with `NotRequired` variant fields for the once-seen-now-away / home / never-seen cases. `_parse` casts the decoded JSON with an `isinstance(dict, ...)` guard. Replaces the stringly-typed field-name coupling flagged as deliberate non-decision — narrow TypedDict costs less than the audit-log consumer drifting
- 📝 **`logging.py` types tightened**: `file: TextIO`, processors typed as `EventDict` / `WrappedLogger` via `structlog.typing`. Eliminates the last `Any` leaks in the logging boundary
- 🛠️ **CI gate now load-bearing.** `continue-on-error: true` removed from `.github/workflows/ci.yml` — `scripts/check.sh` (ruff + ruff format + pyright strict + pytest) must pass for the build to go green. Was tolerated as non-fatal during the H5/logging/monitor typing sweep
- 📐 **Pyright strict mode scoped to `src/`.** Tests are type-checked in basic (non-strict) mode — test fixtures and doubles deliberately take shortcuts the production code cannot
